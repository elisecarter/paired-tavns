...
def preprocess_pupil_data(pupil_df, neon_recording_path=None):
    ...
    if neon_recording_path:
        preprocess_recording, blink_detection_pipeline = _load_blink_modules()
        if preprocess_recording is not None and blink_detection_pipeline is not None:
            left_eye_images, right_eye_images, ts = preprocess_recording(neon_recording_path)
...
def preprocess_ino_data(daq_df):
    ...
    for col in data_cols:
        ...
        if channel_hint == 'ECG':
            ...
            beatTimes = np.asarray(t, dtype=float)[nn_peaks]
