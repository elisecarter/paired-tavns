...
utils::globalVariables(c("subject", "condition", "congruent", "contrast",
                         "value", "rt_mean", "acc_mean", "score_mean"))
...
preprocess_data <- function(df, experiment_type) {
  df2 <- df %>% filter(.data$condition != "practice")

  if (identical(experiment_type, "StroopSquared")) {
    # StroopSquared uses compound trial labels; map the fully congruent
    # and fully incongruent variants to simple labels used in plots.
    # Update these strings if your raw trial_type values differ.
    df2 <- df2 %>%
      mutate(
        ...
        congruent = factor(congruent, levels = c("congruent", "incongruent", "responses incongruent", "cue incongruent"))
      )
  } else {
    df2 <- df2 %>%
      mutate(
        ...
        congruent = factor(congruent, levels = c("congruent", "incongruent"))
      )
  }
...
#------------------------------------------------------------
# Helper to plot per-subject congruency contrasts and group summary
plot_congruency_contrast <- function(subject_summary, metric_prefix, metric_label,
                                    condition_colors, experiment_type = NULL) {
  # metric_prefix examples: "rt", "acc", "score" where subject_summary
  # has columns like rt_mean, acc_mean, score_mean
  mean_col <- paste0(metric_prefix, "_mean")
  # For StroopSquared we only want the fully congruent vs fully incongruent
  # trial types for contrast plotting. If experiment_type indicates
  # StroopSquared, filter subject_summary accordingly before pivoting.
  if (!is.null(experiment_type) && identical(experiment_type, "StroopSquared")) {
    subject_summary <- subject_summary %>%
      dplyr::filter(.data$congruent %in% c("congruent", "incongruent"))
  }

  subj_wide <- subject_summary %>%
    mutate(value = .data[[mean_col]]) %>%
    select(subject, condition, congruent, value) %>%
    pivot_wider(names_from = c(congruent, condition), values_from = value)

  # compute per-subject contrasts (incongruent - congruent) within each condition
  # resulting columns will be like incongruent_sham, congruent_sham, etc.
  contrast_list <- list()
  conditions <- unique(as.character(subject_summary$condition))
  for (cond in conditions) {
    incon_col <- paste0("incongruent_", cond)
    cong_col <- paste0("congruent_", cond)
    if (all(c(incon_col, cong_col) %in% names(subj_wide))) {
      # compute contrast (incongruent - congruent) for this condition
      contrast_vals <- subj_wide[[incon_col]] - subj_wide[[cong_col]]
  contrast_list[[cond]] <- tibble::tibble(subject = subj_wide$subject,
              contrast = contrast_vals,
              condition = cond)
    }
  }

  # combine contrasts into long format for plotting
  if (length(contrast_list) == 0) {
    contrasts_long <- tibble(subject = character(), contrast = double(), condition = character())
  } else {
    contrasts_long <- bind_rows(contrast_list)
  }

  # paired t-tests comparing contrasts across conditions if there are two
  t_results <- NULL
  w_results <- NULL
  diff_plot <- NULL
  if (length(unique(contrasts_long$condition)) == 2) {
    wide <- contrasts_long %>% select(.data$subject, .data$condition, .data$contrast) %>%
      pivot_wider(names_from = .data$condition, values_from = .data$contrast)
    if (all(c("taVNS", "sham") %in% names(wide))) {
      t_results <- t.test(wide$taVNS, wide$sham, paired = TRUE)
      w_results <- suppressWarnings(wilcox.test(wide$taVNS, wide$sham, paired = TRUE, exact = FALSE))
      diff_vals <- wide$taVNS - wide$sham
      diff_plot <- plot_difference_distribution(diff_vals,
        title = paste0("Paired Differences: Congruency Contrast (", metric_label, ")"),
        x_label = "taVNS - sham contrast")
      if (!is.null(diff_plot)) print(diff_plot)
    }
  }

  # plot per-subject contrasts with group mean+sem
  group_sum <- contrasts_long %>%
    group_by(.data$condition) %>%
    summarise(mean = mean(.data$contrast, na.rm = TRUE),
              sem = sd(.data$contrast, na.rm = TRUE) / sqrt(dplyr::n()),
              .groups = "drop")

  # Adjust x-axis labels for StroopSquared to use 'fully congruent' names
  x_lab <- "Condition"

  p <- ggplot(contrasts_long, aes(x = .data$condition, y = .data$contrast)) +
    geom_line(aes(group = .data$subject), color = "gray60", alpha = 0.4, position = position_jitter(width=0.1, height=0)) +
    geom_errorbar(data = group_sum, aes(x = .data$condition, ymin = .data$mean - .data$sem, ymax = .data$mean + .data$sem, color = .data$condition), width = 0.1, linewidth = 1, inherit.aes = FALSE) +
    geom_point(data = group_sum, aes(x = .data$condition, y = .data$mean, color = .data$condition), inherit.aes = FALSE) +
    scale_color_manual(values = condition_colors) +
    labs(title = paste0("Per-subject Congruency Contrast: ", metric_label),
         y = paste0("Incongruent - Congruent (", metric_label, ")"),
         x = x_lab)

  list(plot = p, t_test = t_results, wilcox = w_results)
}

#------------------------------------------------------------
# Helper: plot paired difference distribution for any paired t-test
plot_difference_distribution <- function(diff_values, title, x_label) {
  diff_values <- diff_values[is.finite(diff_values)]
  if (length(diff_values) == 0) {
    return(NULL)
  }
  df_diff <- tibble::tibble(diff = diff_values)
  ggplot(df_diff, aes(x = "", y = diff)) +
    geom_boxplot(width = 0.2, fill = "#ccccff", color = "#333333") +
    geom_point() +
    geom_hline(yintercept = mean(diff_values, na.rm = TRUE),
               linetype = "dashed", color = "#333333") +
    geom_hline(yintercept = 0, linetype = "dotted", color = "#000000") +
    labs(title = title, x = NULL, y = x_label) +
...
plot_subject_summary <- function(subject_summary, group_summary, trial, type, condition_colors) {
  ...
  p <- ggplot(df_subj, aes(x = .data$condition)) +
    geom_line(aes(y = .data[[subj_y]], group = .data$subject),
      alpha = 0.4, color = "gray40", position = position_jitter(width=0.1, height=0)
    ) +
    geom_errorbar(
      data = df_grp,
...
run_analysis <- function(experiment_type) {
  ...
  # ------------------------------------------------------------
  # Compute per-subject congruency contrasts (incongruent - congruent)
  # for rt, accuracy, and score, plot them, and run paired t-tests
  contrast_results <- list()
  metrics <- list(rt = "RT (s)", acc = "Accuracy", score = "Score")
  for (m in names(metrics)) {
    res <- plot_congruency_contrast(subject_summary,
                                    metric_prefix = m,
                                    metric_label = metrics[[m]],
                                    condition_colors = condition_colors,
                                    experiment_type = experiment_type)
    if (!is.null(res)) {
      contrast_results[[m]] <- res$t_test
      if (!is.null(res$t_test)) {
        t_test[[paste0("contrast_", m)]] <- res$t_test
      }
      if (!is.null(res$wilcox)) {
        wilcox[[paste0("contrast_", m)]] <- res$wilcox
      }
      print(res$plot)
    }
  }

 # wilcox signed rank on RT by congruency (conditions pooled)
  if ("rt" %in% analysis_plan$trial_metrics) {
    pivot_wilcox <- df_correct %>%
      dplyr::select(.data$subject, .data$congruent, .data$condition, .data$block, .data$trial_number, .data$rt) %>%
      tidyr::pivot_wider(names_from = .data$congruent, values_from = .data$rt)
    if (all(c("congruent", "incongruent") %in% names(pivot_wilcox))) {
      wilcox[["congruency"]] <- wilcox.test(pivot_wilcox$congruent, pivot_wilcox$incongruent, exact = TRUE)
    }
  }
    ...
    if ("rt" %in% analysis_plan$trial_metrics) {
      ...
      if (all(c("taVNS", "sham") %in% names(pivot_wilcox))) {
        wilcox[[as.character(trial)]] <- wilcox.test(pivot_wilcox$taVNS, pivot_wilcox$sham, exact = TRUE)
      }
      ...
      if (all(c("taVNS", "sham") %in% names(pivot_rt))) {
        diff_vals <- pivot_rt$taVNS - pivot_rt$sham
        diff_plot <- plot_difference_distribution(diff_vals,
          title = paste0("Paired Differences: RT (", trial, ")"),
          x_label = "taVNS - sham (s)")
        if (!is.null(diff_plot)) print(diff_plot)
        t_test[[paste0("rt_", trial)]] <- t.test(pivot_rt$taVNS, pivot_rt$sham, paired = TRUE)
        wilcox[[paste0("rt_", trial)]] <- suppressWarnings(wilcox.test(pivot_rt$taVNS, pivot_rt$sham, paired = TRUE, exact = FALSE))
      }
    ...
    if ("acc" %in% analysis_plan$trial_metrics) {
      ...
      if (all(c("taVNS", "sham") %in% names(pivot_acc))) {
        diff_vals <- pivot_acc$taVNS - pivot_acc$sham
        diff_plot <- plot_difference_distribution(diff_vals,
          title = paste0("Paired Differences: Accuracy (", trial, ")"),
          x_label = "taVNS - sham (proportion)")
        if (!is.null(diff_plot)) print(diff_plot)
        t_test[[paste0("acc_", trial)]] <- t.test(pivot_acc$taVNS, pivot_acc$sham, paired = TRUE)
        wilcox[[paste0("acc_", trial)]] <- suppressWarnings(wilcox.test(pivot_acc$taVNS, pivot_acc$sham, paired = TRUE, exact = FALSE))
      }
  ...
  pivot_all_rt <- aggs$agg_subject_summary %>%
    dplyr::select(.data$subject, .data$rt_mean, .data$condition) %>%
    tidyr::pivot_wider(names_from = .data$condition, values_from = .data$rt_mean)
  if (all(c("taVNS", "sham") %in% names(pivot_all_rt))) {
    diff_vals <- pivot_all_rt$taVNS - pivot_all_rt$sham
    diff_plot <- plot_difference_distribution(diff_vals,
      title = "Paired Differences: Overall RT",
      x_label = "taVNS - sham (s)")
    if (!is.null(diff_plot)) print(diff_plot)
    t_test[["rt_all"]] <- t.test(pivot_all_rt$taVNS, pivot_all_rt$sham, paired = TRUE)
    wilcox[["rt_all"]] <- suppressWarnings(wilcox.test(pivot_all_rt$taVNS, pivot_all_rt$sham, paired = TRUE, exact = FALSE))
  }
  ...
  pivot_all_acc <- aggs$agg_subject_summary %>%
    dplyr::select(.data$subject, .data$acc_mean, .data$condition) %>%
    tidyr::pivot_wider(names_from = .data$condition, values_from = .data$acc_mean)
  if (all(c("taVNS", "sham") %in% names(pivot_all_acc))) {
    diff_vals <- pivot_all_acc$taVNS - pivot_all_acc$sham
    diff_plot <- plot_difference_distribution(diff_vals,
      title = "Paired Differences: Overall Accuracy",
      x_label = "taVNS - sham (proportion)")
    if (!is.null(diff_plot)) print(diff_plot)
    t_test[["acc_allTypes"]] <- t.test(pivot_all_acc$taVNS, pivot_all_acc$sham, paired = TRUE)
    wilcox[["acc_allTypes"]] <- suppressWarnings(wilcox.test(pivot_all_acc$taVNS, pivot_all_acc$sham, paired = TRUE, exact = FALSE))
  }
  ...
  if (all(c("taVNS", "sham") %in% names(pivot_score))) {
    diff_vals <- pivot_score$taVNS - pivot_score$sham
    diff_plot <- plot_difference_distribution(diff_vals,
      title = "Paired Differences: Overall Score",
      x_label = "taVNS - sham (score)")
    if (!is.null(diff_plot)) print(diff_plot)
    t_test[["score"]] <- t.test(pivot_score$taVNS, pivot_score$sham, paired = TRUE)
    wilcox[["score"]] <- suppressWarnings(wilcox.test(pivot_score$taVNS, pivot_score$sham, paired = TRUE, exact = FALSE))
  }
  ...
  trial_counts <- df %>%
    dplyr::group_by(.data$correct,.data$congruent, .data$condition) %>%
